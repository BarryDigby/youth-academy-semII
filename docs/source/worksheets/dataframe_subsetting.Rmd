...
Q3: What is the age of the youngest and eldest patient in the dataset? (Hint use min() and max() on the age column).

Q4: How many patients in the datset have had a stroke? (similar to Q2)
...
Q1: Based on the two dataframes, report the number of females that have had strokes and the number of males that have had strokes. (Hint: use table() on the stroke column for each dataframe).
...
The answer reported in Part 2 could be misleading. If there are more males than females or vice versa, then the result given in Part 2 could be due to sampling bias.
...
* Calculate the number of males that have had a stroke and save it as a variable called 'n_stroke_male'

* Calculate the number of males in the dataset and save it as a variable called 'n_males'

* Divide n_stroke_males/n_males to compute the rate of strokes in men. Save the result to a variable. 
...
Is the rate of strokes higher in males or females? 
...
Just like the analysis of females and males in Part 3, you will need to calculate the rate of strokes in married and single patients. 
...
ggscatter(dataset, x="age", y="bmi", facet.by = "gender", color = "stroke", palette = "lancet", add = "reg.line", conf.int = T, fullrange = T, shape = "stroke", alpha = "stroke")
```